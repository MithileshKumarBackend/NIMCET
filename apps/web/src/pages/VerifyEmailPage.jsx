...
export function VerifyEmailPage() {
  ...
  async function onSubmit(values) {
    try {
      ...
      setError('root', { message: getApiErrorMessage(error, 'Unable to verify email') });
    }
  }
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-20">
      <form onSubmit={handleSubmit(onSubmit)} className="rounded-2xl border bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <p className="mb-6 text-sm text-slate-500">Paste the verification token from your email.</p>
